...
function recognizeFrame(image, spawnedSessionRGBA) {
  ...
  try {
    ...
    if (result.IsTerminal()) spawnedSessionRGBA.Reset();
    result.delete();
    image.delete();
  }
}
...
function createCodeSessionSettings(SE, ENGINE, selected_engine) {
  ...
  if (selected_engine == "shipping_container") {
    console.log("Shipping container enabled");
    // Setting option to enable shipping container recognition
    let engine_name = SE.ToString(SE.EngineSettingsGroup.ShippingContainer);
    console.log(engine_name);
    sessionSettings.SetOption(engine_name + ".enabled", "true");
  }
...
function resultObject(result, image_size = [0, 0]) {
  return {
    requestType: "result",
    docType: "",
    data: getTextFields(result),
    quad: getCodeObjectQuadrangles(result),
    images: [],
  ...
  for (; !co.Equals(result.ObjectsEnd()); co.Advance()) {
    ...
    for (; !tf.Equals(code_object.FieldsEnd()); tf.Advance()) {
      ...
      object_data[code_field_name] = {
        ...
        confidence: code_field.GetConfidence()
      };
    }
    // attr
...
function getAvailableDocList(SE, ENGINE) {
  ...
  if (ENGINE.IsEngineAvailable(SE.CodeEngineType.CodeEngine_ShippingContainer)) {
    console.log("Shipping Container enabled");
    // Setting option to enable shipping container recognition
    let engine_name = SE.ToString(SE.EngineSettingsGroup.ShippingContainer);
    console.log(engine_name);
